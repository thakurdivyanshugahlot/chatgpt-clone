...
# Tavily — web search tool, from tavily.com (free tier: 1,000 credits/mo)
TAVILY_API_KEY="tvly-..."
```
...
- **Web search** — wired into `route.ts` as a tool (`webSearchTool`, backed by Tavily) with `stopWhen: stepCountIs(5)` to allow multi-step search-then-answer turns. Tool-call parts are stored as part of the assistant message's `parts` JSON, same as plain text. Setup below.

### Web search (Tavily)

1. Sign up free at [tavily.com](https://tavily.com) (no card required) → get your key from [app.tavily.com/home](https://app.tavily.com/home) → API Keys. Free tier: 1,000 search credits/month.
2. Add `TAVILY_API_KEY="tvly-..."` to `.env` (see [Environment variables](#2-environment-variables)) and restart `npm run dev` — env vars aren't hot-reloaded.
3. Install the SDK:
   ```bash
   npm install @tavily/core
   ```
4. `features/ai/tools/web-search-tool.ts`:
   ```typescript
   ...
   /**
    * Web search tool backed by Tavily — gives the model current, real-time
    * information for anything beyond its training data (news, prices, recent events).
    */
   export const webSearchTool = tool({
     description:
       "Search the web for current, real-time information. Use this for recent events, current data, prices, weather, or anything that may have changed since your training data.",
   ...
   ```
   `searchDepth: "basic"` costs 1 credit/search; switch to `"advanced"` (2 credits) if results feel too shallow for follow-up questions.
5. `route.ts` already wires this in — no changes needed there:
   ```typescript
   tools: { webSearch: webSearchTool },
   stopWhen: stepCountIs(5), // allows multi-step: search → then answer
   ```
6. Test with a time-sensitive question ("what's the weather today in \<city\>") and confirm a `webSearch` tool-call step appears in your terminal logs before the final answer streams in.

**Alternative — official package:** Tavily also publishes [`@tavily/ai-sdk`](https://www.npmjs.com/package/@tavily/ai-sdk), which exports a ready-made `tavilySearch()` tool for the Vercel AI SDK (reads `TAVILY_API_KEY` automatically, no custom wrapper needed):
```bash
npm install @tavily/ai-sdk
```
```typescript
import { tavilySearch } from "@tavily/ai-sdk";
tools: { webSearch: tavilySearch({ searchDepth: "advanced", includeAnswer: true, maxResults: 5 }) },
```
...
features/
  ...
  ai/
    ...
    tools/web-search-tool.ts  Tavily-backed web search tool
